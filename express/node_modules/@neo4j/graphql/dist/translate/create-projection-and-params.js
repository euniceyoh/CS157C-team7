...
function createProjectionAndParams({ resolveTree, node, context, chainStr, varName, literalElements, resolveType, inRelationshipProjection, }) {
    function reducer(res, field) {
        ...
        if (cypherField) {
            ...
            const apocStr = `${!cypherField.isScalar && !cypherField.isEnum ? `${param} IN` : ""} apoc.cypher.runFirstColumn("${cypherField.statement}", ${apocParamsStr}, ${expectMultipleValues})${apocWhere ? ` ${apocWhere}` : ""}${unionWhere ? ` ${unionWhere} ` : ""}${projectionStr ? ` | ${!referenceUnion ? param : ""} ${projectionStr}` : ""}`;
        ...
        if (relationField) {
            ...
            if (relationField.interface) {
                ...
                if (optionsInput) {
                    ...
                    if (optionsInput.sort) {
                        ...
                        res.projection.push(`${field.alias}: apoc.coll.sortMulti(collect(${field.alias}), [${sorts.join(", ")}])${offsetLimitStr}`);
                        return res;
                    }
                }
                res.projection.push(`${field.alias}: ${!isArray ? "head(" : ""}collect(${field.alias})${offsetLimitStr}${!isArray ? ")" : ""}`);
                return res;
            }
            ...
            const nodeWhereAndParams = createNodeWhereAndParams({
                ...
                node: referenceNode,
                context,
                authValidateStrs: (_f = recurse[2]) === null || _f === void 0 ? void 0 : _f.authValidateStrs,
            });
            if (nodeWhereAndParams[0]) {
